package archetype

import (
	"github.com/ecumeurs/upsilonbattle/battlearena/controller/behavior"
	"github.com/ecumeurs/upsilonbattle/battlearena/controller/behavior/micro"
	"github.com/ecumeurs/upsilontypes/entity/skill"
)

// SneakArchetype — flanker that hunts isolated targets from unexpected angles.
//
// Stack: BackstabSeeker(0.8) → FocusWeakest(0.7) → Flank(0.7) → Baseline(1.0)
//
// @spec-link [[mechanic_ai_controller_archetypes]]
type SneakArchetype struct{}

// Slug returns the canonical identifier used in the archetype registry.
func (SneakArchetype) Slug() string  {
	return "sneak"
}

// Behavior returns the layered behavior stack for this archetype.
func (SneakArchetype) Behavior() *behavior.LayeredBehavior {
	return behavior.NewLayeredBehavior(
		&micro.BackstabSeeker{},
		&micro.FocusWeakest{},
		&micro.Flank{},
		&behavior.AggressiveBehavior{},
	)
}

// AllowedSkillTags returns the skill tags this archetype may equip.
func (SneakArchetype) AllowedSkillTags() []string {
	return []string{"melee", "debuff", "dot", "stun"}
}

// ForbidSkillTags returns the skill tags this archetype may not equip.
func (SneakArchetype) ForbidSkillTags() []string {
	return []string{"aoe", "shield"}
}

// BuildSkillBundle generates numSkills skills filtered by this archetype's tag constraints.
func (SneakArchetype) BuildSkillBundle(grade string, numSkills int) []skill.Skill {
	return buildBundle(SneakArchetype{}.AllowedSkillTags(), SneakArchetype{}.ForbidSkillTags(), grade, numSkills)
}

// StatWeights returns the relative CP allocation weights for each stat.
func (SneakArchetype) StatWeights() StatWeights {
	return StatWeights{
		Attack:   0.35,
		Movement: 0.20,
		HP:       0.20,
		SP:       0.15,
		Defense:  0.10,
	}
}
